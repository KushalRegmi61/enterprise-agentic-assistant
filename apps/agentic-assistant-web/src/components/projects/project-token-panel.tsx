...
export function ProjectTokenPanel({ projectId, role, token }: ProjectTokenPanelProps) {
  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900 p-5 space-y-4">
      <div>
        <p className="text-[10px] uppercase tracking-wide text-indigo-400">Coding-agent access</p>
        <p className="mt-1 text-xs text-slate-400">
          Credentials can access and update only this project and expire after 30 days.
        </p>
      </div>

      {error && <p className="rounded-lg border border-rose-900 bg-rose-950/40 p-3 text-xs text-rose-300">{error}</p>}
      ...
      {created && (
        <div className="rounded-lg border border-amber-700 bg-amber-950/30 p-4 space-y-3">
          ...
          <code className="block break-all rounded bg-slate-950 p-3 text-[11px] text-slate-200">{created.token}</code>
    ...
    "project-status": {
      ...
      {role === "lead" && (
        <form onSubmit={handleCreate} className="flex gap-2">
          <input value={label} onChange={(event) => setLabel(event.target.value)} maxLength={100} placeholder="Token label, e.g. Claude laptop" className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs" required />
          <button disabled={saving} className="rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold disabled:opacity-50">Generate</button>
        </form>
      )}
      ...
      {loading ? <p className="text-xs text-slate-500">Loading credentials…</p> : tokens.length === 0 ? <p className="text-xs text-slate-500">No credentials generated.</p> : (
        <div className="space-y-2">
          {tokens.map((item) => {
            ...
            return (
              <div key={item.id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-800 px-3 py-3 text-xs">
                <div><p className="font-semibold text-slate-200">{item.label}</p><p className="text-[10px] text-slate-500">Expires {formatDate(item.expires_at)} · Last used {formatDate(item.last_used_at)}</p></div>
                <div className="flex items-center gap-2"><span className={inactive ? "text-slate-500" : "text-emerald-400"}>{item.revoked_at ? "Revoked" : inactive ? "Expired" : "Active"}</span>{!item.revoked_at && <button type="button" disabled={saving} onClick={() => void handleRevoke(item)} className="text-rose-400 underline disabled:opacity-50">Revoke</button>}</div>
              </div>
            );
          })}
        </div>
